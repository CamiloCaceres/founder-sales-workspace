---
name: draft-proposal
description: Use when the user says "draft a proposal for {Acme}" / "one-pager proposal for {Acme}" — writes a one-pager proposal with problem statement, scope, pricing, terms, success metrics. Stays inside the playbook's pricing stance; never promises below the non-negotiable.
---
...
2. **Read pricing.** `config/pricing.json` (AE-scoped mirror) or the
   playbook's pricing-stance section. Know the bands, the discount
   policy, and the non-negotiable. **Never draft below the
   non-negotiable.** If the deal requires that, write UNKNOWN and
   flag for approval.
